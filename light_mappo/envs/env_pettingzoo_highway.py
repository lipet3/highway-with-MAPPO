...
# # 文件名: light_mappo/envs/env_pettingzoo_highway.py (极简版)
#
# from pettingzoo.utils import ParallelEnv
# from gymnasium import spaces
# import numpy as np
...
#
# class HighwayParallelEnv(ParallelEnv):
#     metadata = {"render_modes": ["human", "rgb_array"], "name": "MARL_v0"}
#
#     def __init__(self, num_agents=4, render_mode=None):
#         self.possible_agents = [f"agent_{i}" for i in range(num_agents)]
#         self.agents = self.possible_agents[:]
#
#         # 配置中只保留与环境结构和行为相关的核心参数
#         config = {
...
#         }
#         if render_mode:
#             config.update({"screen_width": 1600, "screen_height": 300})
#
#         self.env = gym.make("MARL-v0", render_mode=render_mode, config=config)
#
#         obs_shape = (int(np.prod(self.env.observation_space[0].shape)),)
#         self.observation_spaces = {
#             agent: spaces.Box(low=-np.inf, high=np.inf, shape=obs_shape, dtype=np.float32)
#             for agent in self.possible_agents
#         }
#         self.action_spaces = {
#             agent: self.env.action_space[0]
#             for agent in self.possible_agents
#         }
#
#     def reset(self, seed=None, options=None):
...
#     def step(self, actions):
#         action_tuple = tuple(actions[agent] for agent in self.agents)
#         obs_tuple, reward, terminated, truncated, info = self.env.step(action_tuple)
#
#         observations = {
#             agent: obs_tuple[i].flatten().astype(np.float32)
#             for i, agent in enumerate(self.agents)
...
#         infos = {agent: {} for agent in self.agents}
#
#         # PettingZoo 的标准做法：如果 episode 结束，清空 agents 列表
