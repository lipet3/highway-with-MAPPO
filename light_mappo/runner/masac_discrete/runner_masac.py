...
#         self.base_save_dir = r"C:\Users\z8603\Desktop\study\highway with mappo\light_mappo\results\HighwayEnv\highway\masac"
#         os.makedirs(self.base_save_dir, exist_ok=True)
#         self.run_dir = self._prepare_run_dir(self.base_save_dir)  # 形如 ...\masac\run1
#         print(f"✅ 本次训练输出目录: {self.run_dir}")
#
#     # 创建 run1/run2/... 的递增目录
#     def _prepare_run_dir(self, base_dir):
...
#             i += 1
#
#     def _state(self, obs):  # (A,Do) -> (S,)
#         return obs.reshape(-1)
#
#     # ---------- EMA 工具 ----------
#     def _ema_series(self, xs, alpha):
#         """返回一维列表 xs 的 EMA 列表；xs 可为 list[float] 或 1D np.array"""
#         if xs is None or len(xs) == 0 or alpha <= 0 or alpha >= 1:
#             return None
#         out = []
#         m = None
#         for v in xs:
...
#     def _ema_2d(self, arr2d, alpha):
#         """
#         对形如 (T, A) 的二维数组做每列 EMA，返回 (T, A) 的 numpy 数组（缺失返回 None）。
#         """
#         if arr2d is None or len(arr2d) == 0 or alpha <= 0 or alpha >= 1:
#             return None
#         arr2d = np.asarray(arr2d, dtype=np.float32)
#         T, A = arr2d.shape[0], arr2d.shape[1]
#         out = np.zeros_like(arr2d)
#         for j in range(A):
#             m = None
#             for t in range(T):
...
#     # ---------- 环境剥壳 & 保存奖励曲线 ----------
#     def _dig_core_env(self, obj):
#         """向下剥壳找到 MARLEnv（带 save_reward_curves 的那个）。"""
#         seen = set()
#         stack = [getattr(obj, "env", obj)]
#         core = None
#         while stack:
...
#                 continue
#             seen.add(id(cur))
#             # 优先：直接存在 save_reward_curves 方法
#             if hasattr(cur, "save_reward_curves"):
#                 core = cur
#                 break
#             # 退而求其次：有 controlled_vehicles 和 road 的一般就是 MARLEnv
#             if hasattr(cur, "controlled_vehicles") and hasattr(cur, "road"):
#                 core = cur
#                 # 不 break，继续看看有没有 save_reward_curves 的更底层
#             for name in ("env", "unwrapped", "par_env", "aec_env", "raw_env", "_env", "wrapped_env", "environment"):
#                 if hasattr(cur, name):
#                     nxt = getattr(cur, name)
#                     if isinstance(nxt, (list, tuple)):
...
#     def _save_reward_curves_if_possible(self, force: bool = False, ema_alpha: float = 0.2):
#         """
#         调环境自带的 save_reward_curves：
#         - 原始曲线 + EMA 曲线 + CSV
#         - 默认每隔一段时间保存一次；force=True 时无条件保存
#         保存到 self.run_dir
#         """
#         core = self._dig_core_env(self.env)
#         if core is None or not hasattr(core, "save_reward_curves"):
#             if force:
#                 print("[WARN] 未找到 MARLEnv 或缺少 save_reward_curves，无法保存奖励曲线。")
#             return
#         try:
#             raw_path, ema_path = core.save_reward_curves(
...
#                 ema_alpha=ema_alpha,
#             )
#             if force or (raw_path or ema_path):
#                 print(f"[OK] 奖励曲线已更新：{self.run_dir}")
#         except Exception as e:
#             if force:
#                 print(f"[WARN] 保存奖励曲线失败：{e}")
#
#     # ---------- 训练实时曲线 ----------
#     def render_train(self, ep, team_mean, per_agent_returns):
#         """实时绘制训练曲线（叠加 EMA）"""
...
#             plt.ion()
#             self.fig, self.axs = plt.subplots(2, 1, figsize=(8, 6))
#             self.ax_team = self.axs[0]
#             self.ax_agents = self.axs[1]
#             self.plot_initialized = True
#
#         # 记录数据
#         self.ep_list.append(ep)
...
#         self.agent_returns.append(per_agent_returns)
#
#         agents_np = np.array(self.agent_returns)  # (T, A)
#         T = len(self.ep_list)
#
#         # ---- Team mean ----
#         self.ax_team.clear()
...
#         self.ax_agents.clear()
#         for i in range(self.A):
#             self.ax_agents.plot(self.ep_list, agents_np[:, i], label=f"Agent {i} (raw)")
#         # sm_agents = self._ema_2d(agents_np, self.ema_alpha)
#         # if sm_agents is not None and sm_agents.shape[0] == T:
#         #     for i in range(self.A):
#         #         self.ax_agents.plot(self.ep_list, sm_agents[:, i], linestyle="--", label=f"Agent {i} EMA")
#         self.ax_agents.set_title("Per-Agent Return")
#         self.ax_agents.set_xlabel("Episode")
#         self.ax_agents.set_ylabel("Return")
#         self.ax_agents.grid(True)
...
#     # ---------- 单回合 ----------
#     def run_one_episode(self, greedy=False, ep_idx=0):
#         obs = self.env.reset()  # (A,Do)
#         s = self._state(obs)
#         try:
#             avail = self.env.available_actions()  # (A,NA) or None
#             if avail is None:
#                 avail = np.ones((self.A, self.NA), dtype=np.float32)
#         except Exception:
#             avail = np.ones((self.A, self.NA), dtype=np.float32)
#
#         per_agent_returns = np.zeros(self.A, dtype=np.float32)
#
#         for t in range(self.max_len):
#             a_idx = self.pi.select_actions(obs, avail=avail, greedy=greedy)  # (A,)
#
#             # ---------- 仅统计用：如果 policy 仍然吐出非法动作，这里会看见 ----------
#             try:
#                 mask_ok = avail[np.arange(self.A), a_idx] > 0.5
#                 if not np.all(mask_ok):
#                     bad = np.where(~mask_ok)[0].tolist()
#                     print(f"[RUNCHK] get illegal actions from policy at agents={bad}")
#             except Exception:
#                 # avail 不是 numpy/shape 不对就忽略统计
#                 pass
#             # ----------------------------------------------------------------
#
#
#             a_onehot = np.eye(self.NA, dtype=np.float32)[a_idx]  # 给 wrapper 用 one-hot
#
#             obs2, rew, done, infos = self.env.step(a_onehot)
#             s2 = self._state(obs2)
#
#             try:
#                 avail2 = self.env.available_actions()
#                 if avail2 is None:
#                     avail2 = np.ones((self.A, self.NA), dtype=np.float32)
#             except Exception:
#                 avail2 = np.ones((self.A, self.NA), dtype=np.float32)
#
#             team_r = float(np.mean(rew))
...
#
#         team_mean = float(np.mean(per_agent_returns))
#         # 环境自身也会打印 [EP SUM]/[EP CMP]（来自 MARL.py）
#         print(f"[EP SUM] ep={ep_idx} | team_mean={team_mean:.2f} | per-agent={per_agent_returns.round(2).tolist()}")
#
#         return team_mean, per_agent_returns
#
...
#         print(f"✅ 最终模型已保存到: {actor_final} | {q1_final} | {q2_final}")
#
#         # 训练结束 — 强制再保存一次“环境内置奖励曲线（原始+EMA+CSV）”
#         self._save_reward_curves_if_possible(force=True, ema_alpha=self.ema_alpha)
